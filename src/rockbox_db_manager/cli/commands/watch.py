...
class MusicDirectoryEventHandler(FileSystemEventHandler):
    ...
    def __init__(self, music_path: Path, args: argparse.Namespace, console: Console):
        ...
        self.debounce_seconds = 2  # Wait 2 seconds after last change before regenerating
        
    def should_process_file(self, path: str) -> bool:
        """Check if file is a music file that should trigger regeneration."""
        music_extensions = {'.mp3', '.flac', '.ogg', '.m4a', '.mp4', '.wma', '.wav', '.aac', '.opus'}
        path_obj = Path(path)
    ...
    def on_any_event(self, event: FileSystemEvent) -> None:
        ...
        file_path = Path(event.src_path).relative_to(self.music_path) if self.music_path in Path(event.src_path).parents else Path(event.src_path).name
...
def cmd_watch(args: argparse.Namespace) -> None:
    ...
    console.print("[bold yellow]Performing initial database generation...[/bold yellow]\n")
